...
Parser::Parser(const char *xml_path) {
...
void Parser::initialise_roads_and_vehicles(SimulationModel* simulationModel) {
    ...
    for (TiXmlElement *element = root->FirstChildElement();             //Loop over all tags should only contain
        ...
        if (element->FirstChild() != nullptr) {
            ...
            else rejected_tag_count++;       //If the tag does contain something else it wont be processed!
            //todo what to do with rejected tags?
        }
    }
...
void Parser::initialise_roads(std::vector<TiXmlElement *> elements_of_roads, SimulationModel *simulationModel) {
    for (const auto &road : elements_of_roads) {
        ...
        for (TiXmlElement *attribute = road->FirstChildElement();
            ...
            else if (attribute_name == "lengte")length = static_cast<unsigned int>(std::stoi(attribute->GetText()));
        }
...
void Parser::initialise_vehicles(std::vector<TiXmlElement *> elements_of_vehicles, SimulationModel *simulationModel) {
    for (auto element  :elements_of_vehicles) {
        ...
        for (TiXmlElement *attribute = element->FirstChildElement();
            ...
            else if (element_name == "positie")position = static_cast<unsigned int>(std::stoi(attribute->GetText()));
        ...
        if (road != nullptr and !collides)simulationModel->add_vehicle(new Vehicle(speed,position,license_plate,type,road));
   }
}
